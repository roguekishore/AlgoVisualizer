...
const LISVisualizer = () => {
  ...
  const arrayBoxClass = (idx) => {
    if (idx === state.i) return "bg-accent-primary/80 shadow-lg ring-2 ring-blue-400";
    if (idx === state.j) return "bg-orange/80 shadow-lg ring-2 ring-amber-400";
  ...
  return (
    <div className="px-6 py-8 max-w-7xl mx-auto relative bg-theme-primary min-h-screen text-theme-primary">
      <div className="absolute top-8 right-12 w-96 h-96 bg-success/8 rounded-full blur-3xl pointer-events-none" />

      <header className="relative z-10 mb-12 text-center">
        <h1 className="text-5xl md:text-6xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-success400 via-teal-400 to-accent-primary400">
          Longest Increasing Subsequence
        </h1>
        <p className="text-theme-secondary mt-2 text-lg">Find the length of the longest strictly increasing subsequence</p>
      </header>
      ...
      <section className="mb-6 z-10 relative">
        <div className="flex flex-col md:flex-row gap-3 items-center">
          <input
            ...
            disabled={isLoaded}
            className="flex-1 p-3 rounded-xl bg-theme-secondary border border-theme-primary text-theme-primary font-mono focus:ring-2 focus:ring-green-400"
          ...
          {!isLoaded ? (
            <button
              onClick={load}
              className="px-5 py-3 rounded-xl bg-success-light hover:bg-success/40 transition text-theme-primary font-bold shadow-lg"
            >
              Load & Visualize
            </button>
          ) : (
            <>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-2 rounded-full bg-theme-tertiary hover:bg-success-hover disabled:opacity-40 transition"
                >
                  <ArrowLeft />
                </button>
                <button
                  onClick={() => setPlaying((p) => !p)}
                  className="px-3 py-2 rounded-full bg-theme-tertiary hover:bg-success-hover transition"
                >
                  {playing ? <Pause /> : <Play />}
                </button>
                <button
          ...
          {LANG_TABS.map((lang) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium text-sm ${
                activeLang === lang
                  ? "bg-success-light text-success ring-1 ring-green-400"
      ...
      {!isLoaded ? (
        <div className="mt-10 text-center text-theme-tertiary italic">
          Enter an array and click <span className="text-success font-semibold">Load & Visualize</span>
        </div>
      ) : (
          ...
          <section className="lg:col-span-2 flex flex-col gap-6">
            <div className="p-4 bg-theme-tertiary/50 rounded-xl border border-theme-primary/60">
              ...
              <div className="flex gap-2 flex-wrap">
                {array.map((val, idx) => (
                  <div
                    key={idx}
                    className={`flex flex-col items-center justify-center w-16 h-16 rounded-lg ${arrayBoxClass(idx)} border border-theme-secondary transition-all`}
                  >
                    <div className="text-xs text-theme-tertiary">{idx}</div>
                    <div className="text-lg font-mono text-theme-primary font-bold">{val}</div>
                  </div>
                ))}
              </div>
              <div className="mt-3 flex gap-4 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-theme-tertiary">Current (i)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 bg-orange rounded"></div>
                  <span className="text-theme-tertiary">Comparing (j)</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="col-span-2 p-4 bg-theme-tertiary/50 rounded-xl border border-theme-primary/60">
                ...
                {state.sequences && state.i !== null && state.sequences[state.i] && (
                  <div className="mt-3 p-3 bg-success-light rounded-lg border border-success/30">
                    <div className="text-xs text-success font-semibold mb-1">LIS ending at index {state.i}:</div>
                    <div className="flex gap-1 flex-wrap">
                      {state.sequences[state.i].map((num, idx) => (
                        <div key={idx} className="bg-success-hover/80 text-theme-primary px-2 py-1 rounded-md font-mono text-xs">
                          {num}
                        </div>
                      ))}
                    </div>
              ...
              <div className="p-4 bg-theme-tertiary/50 rounded-xl border border-theme-primary/60">
                ...
                <div className="text-3xl font-mono text-success">{state.maxLen ?? 1}</div>
                <div className="mt-2 text-xs text-theme-tertiary">LIS Length: {state.maxLen ?? 1}</div>
              </div>
            </div>
            ...
            <div className="p-4 bg-theme-tertiary/50 rounded-xl border border-theme-primary/60">
              ...
              <div className="mt-3 text-sm text-theme-secondary space-y-2">
                ...
                <div><strong>Space:</strong> <span className="font-mono text-teal300">O(N)</span></div>
