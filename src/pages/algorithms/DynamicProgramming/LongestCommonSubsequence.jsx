...
const LCSVisualizer = () => {
  ...
  const cellClass = (i, j) => {
    ...
    if (i === state.i && j === state.j) return "bg-accent-primary/80 shadow-lg";
  ...
  const charClass = (str, idx, isFirst) => {
    ...
    return `w-10 h-10 flex items-center justify-center rounded-lg font-mono font-bold text-theme-primary ${
      idx === activeIdx ? "bg-accent-primary shadow-lg" : "bg-theme-elevated"
    }`;
  };

  return (
    <div className="px-6 py-8 max-w-7xl mx-auto relative">
      <div className="absolute top-8 left-1/2 -translate-x-1/2 w-[36rem] h-[36rem] bg-teal/8 rounded-full blur-3xl pointer-events-none" />

      <header className="relative z-10 mb-12 text-center">
        <h1 className="text-5xl md:text-6xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-teal400 via-blue-400 to-purple400">
          Longest Common Subsequence
        </h1>
        <p className="text-theme-secondary mt-2 text-lg">Visualize the DP table & LCS construction</p>
      </header>
      ...
      {!isLoaded ? (
        <div className="mt-10 text-center text-theme-tertiary italic">
          Enter two strings and click <span className="text-teal font-semibold">Load & Visualize</span>
        </div>
      ) : (
          ...
          <section className="lg:col-span-2 flex flex-col gap-6">
                      ...
                      state.dp.map((row, i) => (
                        <tr key={i}>
                          {row.map((val, j) => (
                            <td
                              key={j}
                              className={`px-2 py-1 text-center ${cellClass(i, j)} text-theme-primary border border-theme-secondary`}
                            >
                              {val}
                            </td>
                          ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-4 bg-theme-tertiary/50 rounded-xl border border-theme-primary/60">
                ...
                <div className="text-3xl font-mono text-success">{state.lcsLength ?? 0}</div>
                <div className="mt-2 text-xs text-theme-tertiary">LCS Length: {state.lcsLength ?? 0}</div>
                <div className="mt-3">
                  <div className="text-xs text-theme-secondary mb-1">LCS String:</div>
                  <div className="bg-orange600/80 text-theme-primary px-3 py-2 rounded-md font-mono text-sm">
                    {state.lcsResult || '""'}
                  </div>
                </div>
              </div>
